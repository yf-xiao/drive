;! function(a, b) {
    ...
    f.pt.set = function(a) {
        ...
        p.fix && "" === p.offset[0] && !p.shift && d.resize( 
        function() {
            f.css({
    ...
    f.pt.callback = function() {
        ...
        a.config.success(b),
        mbox.ie6 && a.IE6(b),
        b.find(".jm-box-close").click(
        function() {
            d.close(a.index),
            mbox.close(a.index)
        }),
        b.find(".jm-box-yes").click(
        function() {
            d.yes ? d.yes(a.index) : e.yes(a.index),
			mbox.close(a.index)
        }),
        b.find(".jm-box-no").click(
        function() {
            d.no ? d.no(a.index) : e.no(a.index),
            mbox.close(a.index)
        }),
        a.config.shadeClose && c("#jm-box-shade" + a.index).click(
        function() {
            mbox.close(a.index)
        }),
        b.find(".jm-box-min").click(
        function() {
            mbox.min(a.index, d),
            d.min && d.min(b)
        }),
        b.find(".jm-box-max").click(
        function() {
    ...
    i.run = function() {
        ...
        (new Image).src = mbox.path + "jm-box-ico0.png"
    },
    g = "../../init/jquery",
    a.seajs ? define([g], 
    function(a, b, c) {
